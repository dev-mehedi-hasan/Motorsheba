    ...
    <section class="mb-4 pt-3">
        <div class="container">
            <ul class="breadcrumb bg-transparent p-0">
                ...
                @endif
                @if($detailedProduct->category->id)
                    @if(\App\Category::find($detailedProduct->category->id)->parent_id == 0)
                    <li class="text-dark fw-600 breadcrumb-item">
                        <a class="text-reset" href="{{ route('products.category', \App\Category::find($detailedProduct->category->id)->slug) }}">{{ \App\Category::find($detailedProduct->category->id)->getTranslation('name') }}</a>
                    </li>
                    
                    @else
                        @if(\App\Category::find(\App\Category::find($detailedProduct->category->id)->parent_id)->parent_id == 0)
                            <li class="text-dark breadcrumb-item">
                                <a class="text-reset" href="{{ route('products.category', \App\Category::find(\App\Category::find($detailedProduct->category->id)->parent_id)->slug) }}">{{ \App\Category::find(\App\Category::find($detailedProduct->category->id)->parent_id)->getTranslation('name') }}</a>
                            </li>
                            <li class="text-dark fw-600 breadcrumb-item">
                                <a class="text-reset" href="{{ route('products.subcategory', ['category_id'=>\App\Category::find($detailedProduct->category->id)->parent_id,'subcategory_slug' => \App\Category::find($detailedProduct->category->id)->slug]) }}">{{ \App\Category::find($detailedProduct->category->id)->getTranslation('name') }}</a>
                            </li>
                        @else
                            @if(\App\Category::find(\App\Category::find(\App\Category::find($detailedProduct->category->id)->parent_id)->parent_id)->parent_id == 0)
                                <li class="text-dark breadcrumb-item">
                                    <a class="text-reset" href="{{ route('products.category', \App\Category::find(\App\Category::find(\App\Category::find($detailedProduct->category->id)->parent_id)->parent_id)->slug) }}">{{ \App\Category::find(\App\Category::find(\App\Category::find($detailedProduct->category->id)->parent_id)->parent_id)->getTranslation('name') }}</a>
                                </li>
                                <li class="text-dark breadcrumb-item">
                                    <a class="text-reset" href="{{ route('products.subcategory', ['category_id'=>\App\Category::find(\App\Category::find($detailedProduct->category->id)->parent_id)->parent_id,'subcategory_slug' => \App\Category::find(\App\Category::find($detailedProduct->category->id)->parent_id)->slug]) }}">{{ \App\Category::find(\App\Category::find($detailedProduct->category->id)->parent_id)->getTranslation('name') }}</a>
                                </li>
                                <li class="text-dark fw-600 breadcrumb-item">
                                    <a class="text-reset" href="{{ route('products.subcategory', ['category_id'=>\App\Category::find($detailedProduct->category->id)->parent_id,'subcategory_slug' => \App\Category::find($detailedProduct->category->id)->slug]) }}">{{ \App\Category::find($detailedProduct->category->id)->getTranslation('name') }}</a>
                                </li>
                        ...
                        <div class="text-left">
                            ...
                            <div class="row align-items-center">
                                <div class="col-sm-2">
                                    ...
                                    @if ($detailedProduct->user->user_type == 'seller' && get_setting('vendor_system_activation') == 1)
                                ...
                                <div class="col-sm-10">
                                    @if (get_setting('conversation_system') == 1)
                                            <button class="btn btn-sm btn-primary" onclick="show_chat_modal()">{{ translate('Pre-order inquiry')}}</button>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="row align-items-center">
                                <div class="col-sm-2">
                                    <div class=" my-2">{{ translate('Brand')}}:</div>
                                </div>
                                <div class="col-sm-10">
                            ...
                            <div class="row align-items-center">
                                <div class="col-sm-2">
                                    <div class=" my-2">{{ translate('Category')}}:</div>
                                </div>
                                <div class="col-sm-10">
                                    @if ($detailedProduct->category != null)
                                        ...
                                        @elseif($detailedProduct->category->parent_id != 0)
                                        <a href="{{ route('products.subcategory', ['category_id'=>$detailedProduct->category->parent_id,'subcategory_slug' => $detailedProduct->category->slug]) }}">
                                            {{$detailedProduct->category->name}}
                                        </a>
                                        @endif
                            ...
                            <div class="mt-3">
                                <button type="button" class="btn btn-dark hov-bg-primary border-none mr-2 add-to-cart fw-600" onclick="addToCart()">
                    ...
                    <div class="bg-white shadow-sm mb-3">
                        <div class="position-relative p-3 text-left">
                            ...
                            <div class="text-center border rounded p-2 mt-3">
                                ...
                                <div class=" fs-12">({{ $total }} {{ translate('customer reviews')}})</div>
                            </div>
                        </div>
                        @if($detailedProduct->added_by == 'seller' && get_setting('vendor_system_activation') == 1)
                                        ...
                                        @foreach ($detailedProduct->reviews as $key => $review)
                                            ...
                                            <li class="media list-group-item d-flex">
                                                ...
                                                <div class="media-body text-left">
                                                    ...
                                                    <div class=" mb-2">{{ date('d-m-Y', strtotime($review->created_at)) }}</div>
